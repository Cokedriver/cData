...
local Core = LibStub("AceAddon-3.0"):GetAddon("nData")
...
local PLAYER_CLASSCOLOR = RAID_CLASS_COLORS[select(2, UnitClass("player"))]
...
function Module:CreateStats(self)
	local db = Core.db.profile
	...
	if db.datapanel.classcolor ~= true then
		...
		hexa = ("|cff%.2x%.2x%.2x"):format(PLAYER_CLASSCOLOR.r * 255, PLAYER_CLASSCOLOR.g * 255, PLAYER_CLASSCOLOR.b * 255)
		hexb = "|r"
	end	
	...
	-- Player Armor
	----------------
	if db.datapanel.armor then
		local effectiveArmor
		
		local Stat = CreateFrame('Frame', nil, MainPanel)
		Stat:EnableMouse(true)
		Stat:SetFrameStrata('BACKGROUND')
		Stat:SetFrameLevel(3)

		local Text  = Stat:CreateFontString(nil, 'OVERLAY')
		Text:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		PlacePlugin(db.datapanel.armor, Text)
	...
	if db.datapanel.avd then
		...
		local Text  = Stat:CreateFontString(nil, 'OVERLAY')
		Text:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		PlacePlugin(db.datapanel.avd, Text)
	...
	if db.datapanel.bags then
		...
		local Text = Stat:CreateFontString(nil, 'OVERLAY')
		Text:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		PlacePlugin(db.datapanel.bags, Text)
	...
	if db.datapanel.battleground == true then 
		...
		Text1:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		Text1:SetPoint('LEFT', BGPanel, 30, 0)
		Text1:SetHeight(MainPanel:GetHeight())

		local Text2  = BGPanel:CreateFontString(nil, 'OVERLAY')
		Text2:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		Text2:SetPoint('CENTER', BGPanel, 0, 0)
		Text2:SetHeight(MainPanel:GetHeight())

		local Text3  = BGPanel:CreateFontString(nil, 'OVERLAY')
		Text3:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		Text3:SetPoint('RIGHT', BGPanel, -30, 0)
		Text3:SetHeight(MainPanel:GetHeight())
	...
	-- Call To Arms
	----------------
	if db.datapanel.calltoarms then
		local Stat = CreateFrame('Frame', nil, MainPanel)
		Stat:EnableMouse(true)
		...
		local Text  = Stat:CreateFontString(nil, "OVERLAY")
		Text:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		PlacePlugin(db.datapanel.calltoarms, Text)
	...
	if db.datapanel.coords then
		...
		local Text = Stat:CreateFontString(nil, "OVERLAY")
		Text:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		PlacePlugin(db.datapanel.coords, Text)
	...
	if db.datapanel.dps_text then
		...
		local DPS_FEED = CreateFrame('Frame', nil, MainPanel)
		...
		dText:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		dText:SetText("DPS: ", '0')

		PlacePlugin(db.datapanel.dps_text, dText)
	...
	if db.datapanel.dur then
		...
		local Text  = Stat:CreateFontString(nil, "OVERLAY")
		Text:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		PlacePlugin(db.datapanel.dur, Text)
	...
	if db.datapanel.friends then
		...
		local Text  = Stat:CreateFontString(nil, "OVERLAY")
		Text:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		PlacePlugin(db.datapanel.friends, Text)
	...
	if db.datapanel.guild then
		...
		local Text  = Stat:CreateFontString(nil, "OVERLAY")
		Text:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		PlacePlugin(db.datapanel.guild, Text)
	...
	if db.datapanel.haste then
		...
		local Text  = Stat:CreateFontString(nil, 'OVERLAY')
		Text:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		PlacePlugin(db.datapanel.haste, Text)
	...
	-- Heals Per Second
	--------------------
	if db.datapanel.hps_text then
		local events = {SPELL_HEAL = true, SPELL_PERIODIC_HEAL = true}
		local HPS_FEED = CreateFrame('Frame', nil, MainPanel)
		local player_id = UnitGUID('player')
		local actual_heals_total, cmbt_time = 0
	 
		local hText = HPS_FEED:CreateFontString(nil, 'OVERLAY')
		hText:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		hText:SetText("HPS: ", '0')
	 
		PlacePlugin(db.datapanel.hps_text, hText)
	...
	-- Professions
	---------------
	if db.datapanel.pro then

		local Stat = CreateFrame('Button', nil, MainPanel)
		Stat:RegisterEvent('PLAYER_ENTERING_WORLD')
		Stat:SetFrameStrata('BACKGROUND')
		Stat:SetFrameLevel(3)
		Stat:EnableMouse(true)
		...
		local Text = Stat:CreateFontString(nil, 'OVERLAY')
		Text:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		PlacePlugin(db.datapanel.pro, Text)
			...
			if btn == "LeftButton" then
				if prof1 then
					if (GetProfessionInfo(prof1) == ('Herbalism')) then
						print('|cffCC3333n|rData: |cffFF0000Herbalism has no options!|r')
					elseif(GetProfessionInfo(prof1) == ('Skinning')) then
						print('|cffCC3333n|rData: |cffFF0000Skinning has no options!|r')
					...
					print('|cffCC3333n|rData: |cffFF0000No Profession Found!|r')
				end
	...
	-- Recount
	-----------
	if db.datapanel.recount then 

		local RecountDPS = CreateFrame('Frame', nil, MainPanel)
		...
		Text:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		PlacePlugin(db.datapanel.recount, Text)
	...
	if db.datapanel.spec then
		...
		local Text  = Stat:CreateFontString(nil, 'OVERLAY')
		Text:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		PlacePlugin(db.datapanel.spec, Text)
	...
	if db.datapanel.stat1 then
		...
		local Text  = Stat:CreateFontString(nil, "OVERLAY")
		Text:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		PlacePlugin(db.datapanel.stat1, Text)
	...
	if db.datapanel.stat2 then
		...
		local Text  = Stat:CreateFontString(nil, "OVERLAY")
		Text:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		PlacePlugin(db.datapanel.stat2, Text)
	...
	if db.datapanel.system then
		...
		local Text  = Stat:CreateFontString(nil, "OVERLAY")
		Text:SetFont(db.datapanel.fontNormal, db.datapanel.fontSize,'THINOUTLINE')
		PlacePlugin(db.datapanel.system, Text)
